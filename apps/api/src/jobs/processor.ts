...
export class JobProcessor {
  ...
  static async initialize(): Promise<void> {
    ...
    const queueNames = Object.values(JobType).map(type => this.getQueueName(type));
    ...
    try {
      ...
      switch (job.name as JobType) {
        case JobType.INACTIVITY_CHECK:
          result = await this.processInactivityCheck(job.data as InactivityCheckJobData);
          break;
          
        case JobType.SEND_REMINDER:
          result = await this.processSendReminder(job.data as SendReminderJobData);
          break;
          
        case JobType.EXECUTE_HANDOVER:
          result = await this.processExecuteHandover(job.data as ExecuteHandoverJobData);
          break;
          
        case JobType.CLEANUP_SESSIONS:
          result = await this.processCleanupSessions(job.data as CleanupSessionsJobData);
          break;
