...
import { Database, SuccessorNotification, NewSuccessorNotification, SuccessorNotificationUpdate } from '../types';
...
export class SuccessorNotificationRepository {
  ...
  async update(handoverProcessId: string, successorId: string, data: SuccessorNotificationUpdate): Promise<SuccessorNotification> {
    try {
      const notification = await this.db
  ...
  async findByHandoverProcess(handoverProcessId: string): Promise<SuccessorNotification[]> {
    try {
